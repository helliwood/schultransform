  ...
  <div v-if="dataCookie">
    ...
    >
      ...
      <template v-slot:modal-header>
        ...
        <div class="d-flex justify-content-between w-100 m-1 m-sm-0 p-0">
          <!--Title-->
          <h5 class="py-2 pl-2 m-0 ml-1">{{ getModalTitle }}</h5>
          <b-button class="pr-2 mr-1" variant="light" size="sm" @click="hideCookieBanner">
        ...
        <div class="d-flex w-100 flex-column flex-sm-row justify-content-between">
          <div class="flex-grow-1">
            <b-link
                target="_blank"
                :href="getLink"
                class="lh-base fs-6 text-decoration-none"
            >
              {{ getLinkText }}
            </b-link>
          </div>
          <div class="d-flex flex-column flex-sm-row">
            <b-button
                ...
                class="p-sm-1 ms-sm-1 my-1 my-sm-0"
            >
              {{ getBtnChange }}
            </b-button>
            <b-button
    ...
    myProvider: function (viewOnly) {
      ...
      return promise.then((data) => {
        ...
        if (this.modal) {
          ...
          if (myHtml.item(0)) {
            //keep the original value in the variable
            this.htmlOverflowInitial = myHtml.item(0).style.overflow;
